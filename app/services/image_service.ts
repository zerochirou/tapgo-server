...
export default class ImageService {
  constructor(protected bucket: string = process.env.SUPABASE_BUCKET as string) {}
  ...
  public async uploadBackgroundImage(file: any): Promise<any> {
    ...
    const { error } = await supabase.storage.from(this.bucket).upload(fileName, fileBuffer, {
  ...
  public async removeImage(pathRelative: string) {
    const { data, error } = await supabase.storage.from(this.bucket).remove([pathRelative])
